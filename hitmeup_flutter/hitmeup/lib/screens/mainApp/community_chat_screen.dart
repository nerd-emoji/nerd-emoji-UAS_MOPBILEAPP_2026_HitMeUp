...
class _CommunityChatScreenState extends State<CommunityChatScreen> {
  ...
  int? get _communityId => int.tryParse(_community.id);
  ...
  Future<void> _loadMessages() async {
    ...
    try {
      final messages = await ChatService.fetchCommunityMessages(
        communityId,
  ...
  Future<void> _loadOlderMessages() async {
    ...
    try {
      final olderMessages = await ChatService.fetchCommunityMessages(
        communityId,
        limit: _messagePageSize,
  ...
  Future<void> _pollForNewMessages() async {
    final communityId = _communityId;
    if (communityId == null || _messages.isEmpty) {
      return;
    }
    ...
    try {
      ...
      final newMessages = await ChatService.fetchCommunityMessages(
        communityId,
        limit: _messagePageSize,
  ...
  Future<void> _sendMessage() async {
    ...
    if (userId == null || communityId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Not logged in or invalid community')),
      );
      return;
    }
    ...
    try {
      final message = await ChatService.sendCommunityMessage(
  ...
  Future<void> _pickAndSendGalleryImage() async {
    ...
    try {
      ...
      final sentMessage = await ChatService.sendCommunityImageMessage(
        communityId: communityId,
        senderId: userId,
  ...
  Future<void> _sendPollMessage({
    ...
    try {
      final createdMessage = await ChatService.sendCommunityPollMessage(
        communityId: communityId,
        senderId: userId,
  ...
  Future<void> _voteOnPoll({
    ...
    try {
      await ChatService.sendCommunityPollVote(
        optionId: optionId,
        voterId: voterId,
      );
  ...
  Future<void> _startVoiceRecording() async {
    ...
    try {
      final recordingPath =
          '${Directory.systemTemp.path}${Platform.pathSeparator}community_voice_${DateTime.now().microsecondsSinceEpoch}.m4a';
  ...
  Future<void> _sendVoiceRecording() async {
    ...
    try {
      ...
      final sentMessage = await ChatService.sendCommunityVoiceMessage(
        communityId: communityId,
        senderId: userId,
  ...
  String? _resolveCommunityImageUrl(String? rawUrl) {
    ...
    return _resolveMediaUrl(rawUrl);
  }
  ...
  Map<String, dynamic> _toDisplayMessage(Map<String, dynamic> msg) {
    ...
    return {
      ...
      'senderProfile': msg['senderProfile'] as String?,
      'text': msg['text'] ?? '',
  ...
  Widget _buildAppBar(BuildContext context) {
    final imageUrl = _resolveCommunityImageUrl(_community.imageUrl);
    return Padding(
      padding: EdgeInsets.only(
        ...
        children: [
          ...
          GestureDetector(
            ...
            onTap: () async {
              ...
              if (!mounted || updatedCommunity == null) {
                return;
              }
